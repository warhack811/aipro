...
class TestSafeCallbackExecutor:
    ...
    async def test_successful_callback(self):
        ...
        success = await executor.execute(callback=mock_callback, result=result, job_id=job_id)
    ...
    async def test_callback_retry_on_failure(self):
        ...
        success = await executor.execute(callback=failing_callback, result="test", job_id="job_retry")
    ...
    async def test_callback_all_retries_fail(self):
        ...
        success = await executor.execute(callback=always_failing_callback, result="test", job_id="job_fail")
    ...
    async def test_async_callback_support(self):
        ...
        success = await executor.execute(callback=async_callback, result="async_test", job_id="job_async")
    ...
    async def test_context_tracking(self):
        ...
        await executor.execute(callback=failing_callback, result="test", job_id="job_context", context=context)
...
class TestIntegration:
    ...
    async def test_callback_with_db_operation(self):
        ...
        success = await executor.execute(callback=callback_with_db, result="test", job_id="job_db")
    ...
    async def test_multiple_callbacks_concurrent(self):
        ...
        async def run_callback(i):
            ...
            return await executor.execute(callback=callback, result=f"result_{i}", job_id=f"job_{i}")
