...
class TestChromaDBWhereFilter:
    ...
    async def test_rag_search_uses_where_filter(self):
        ...
        with patch("app.memory.rag._get_rag_collection") as mock_get_col:
            ...
            # Test
            results = search_documents(query="test query", owner="testuser", scope="user", max_items=5)
    ...
    async def test_memory_service_uses_where_filter(self):
        ...
        with patch.object(MemoryService, "_get_collection") as mock_get_col:
            ...
            mock_collection.query.return_value = {
                ...
                "metadatas": [
                    [{"user_id": 1, "is_active": True, "importance": 0.8, "created_at": datetime.now().isoformat()}]
                ],
                "distances": [[0.1]],
            }
            mock_get_col.return_value = mock_collection
            ...
            # Test
            results = await MemoryService.retrieve_relevant_memories(user_id=1, query="test", limit=5)
...
class TestAlembicMigration:
    ...
    def test_create_db_has_deprecation_warning(self):
        ...
        with patch("app.core.database.logger") as mock_logger:
            ...
            warning_calls = [call for call in mock_logger.warning.call_args_list if "CREATE ALL" in str(call)]
