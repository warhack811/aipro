...
async def add_memory(username: str, text: str, importance: float = 0.5, tags: Optional[List[str]] = None) -> MemoryItem:
    """
    ...
    try:
        record = await MemoryService.add_memory(user_id=user_id, text=text, importance=importance, topic=topic)
...
async def list_memories(username: str) -> List[MemoryItem]:
    ...
    try:
        records = await MemoryService.retrieve_relevant_memories(
            user_id=user_id, query="everything about me", limit=100, min_relevance=0.0
        )
        return [_record_to_item(rec) for rec in records]
    except Exception as e:
...
async def update_memory(
    username: str, memory_id: str, text: str, importance: Optional[float] = None
) -> Optional[MemoryItem]:
    """
