...
class MamiException(Exception):
    ...
    def __init__(self, message: str, user_message: Optional[str] = None, status_code: int = 500):
        """
        Args:
...
class AuthenticationError(MamiException):
    ...
    def __init__(self, message: str):
        super().__init__(message=message, user_message="Oturum süren dolmuş veya giriş yapmamışsın.", status_code=401)
...
class SearchError(MamiException):
    ...
    def __init__(self, message: str):
        super().__init__(
            message=message, user_message="İnternet araması şu an yapılamıyor. Lütfen tekrar deneyin.", status_code=503
        )
