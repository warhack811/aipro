...
def full_post_process(
    text: str, options: Optional[Dict[str, Any]] = None, context: Optional[Dict[str, Any]] = None
) -> str:
    """
    ...
    if plugin and plugin.is_enabled():
        ...
        logger.info(
            f"[POST_PATH] using=plugin_enhancement plugin_enabled=True preset={preset} beautify={preset_options.get('enable_beautification')} shaping={preset_options.get('enable_smart_shaping')}"
        )

        enhanced = plugin.process_response(
            text=text, context=context, options=preset_options  # Tam preset config'i geç
        )
