...
class ContextTruncationManager:
    ...
    @staticmethod
    def calculate_message_importance(message: Dict[str, str], position_index: int, total_messages: int) -> float:
        """
        ...
        importance = position_weight * 0.4 + role_weight * 0.2 + length_weight * 0.2 + content_weight * 0.2

        return min(1.0, importance)

    @classmethod
    def truncate_messages_by_importance(
        cls, messages: List[Dict[str, str]], token_budget: int, preserve_system: bool = True
        ...
        for idx, msg in enumerate(content_messages):
            ...
            scored_messages.append({"message": msg, "importance": importance, "tokens": tokens, "index": idx})
    ...
    @classmethod
    def truncate_text_smart(cls, text: str, char_limit: int = DEFAULT_CHAR_LIMIT, add_notice: bool = True) -> str:
        """
    ...
    @classmethod
    def build_context_blocks(cls, sections: List[str], char_limit: int = DEFAULT_CHAR_LIMIT) -> str:
        """
