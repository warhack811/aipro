...
def get_username_from_token(token: str) -> Optional[str]:
    ...
    with get_session() as session:
        stmt = select(Session).where(
            Session.id == token, Session.type == "remember_token", Session.expires_at > datetime.utcnow()
        )
...
def cleanup_expired_tokens() -> int:
    ...
    with get_session() as session:
        stmt = select(Session).where(Session.type == "remember_token", Session.expires_at <= now)
