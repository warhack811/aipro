...
try:
    ...
    celery_app = Celery("mami_async_image", broker="redis://localhost:6379/0", backend="redis://localhost:6379/1")
...
if celery_app:
    ...
    def generate_image_task(
        self, username: str, prompt: str, conversation_id: Optional[str] = None, user_id: Optional[int] = None
    ):
        """
        ...
        try:
            ...
            try:
                ...
                if conversation_id:
                    ...
                    append_message(
                        username=username, conv_id=conversation_id, role="bot", text=text, extra_metadata=meta
                    )
                ...
                try:
                    ...
                    loop.run_until_complete(
                        send_to_user(
                            username,
                            {"type": "image_complete", "conversation_id": conversation_id, "image_path": image_path},
                        )
                    )
                    loop.close()
            ...
            if conversation_id:
                ...
                append_message(username=username, conv_id=conversation_id, role="bot", text=error_text)
