...
class ResponseOrchestrator:
    ...
    def process(
        self, text: str, context: Optional[Dict[str, Any]] = None, options: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        ...
        if options.get("enable_smart_shaping", True) and user_message:
            text, shape_mode, shape_reason = self.plugin.answer_shaper.shape(text, user_message, mode="auto")
            logger.info(
                f"[POST_STAGE] stage=4_shaping mode={shape_mode} reason={shape_reason} preview={text[:200] if text else 'N/A'}..."
            )
        ...
        final_len = len(text)
        logger.info(f"[ORCHESTRATOR] Complete: {original_len} -> {final_len} chars, " f"shape={shape_mode}")
